...
<div class="actions columns large-2 medium-3">
    <h3><?= __('Actions') ?></h3>
    <ul class="side-nav">
        <li><?= $this->Html->link(__('List Blog'), ['action' => 'index']) ?></li></li>
    </ul>
</div>
   ...
   <?= $this->Form->create($blog,["type"=>"file"]); ?>
    <fieldset>
        ...
        <?php
            ...
            echo "<tr><td>Type</td><td>".$this->Form->input('type',array('label'=>false,'class'=>'form-control'))."</td></tr>";
            /**************Listing Image**************/
            echo "<tr><td>Listing Image</td><td>".$this->Form->input('listing_image_new',['type'=>'file','label'=>false])."<br>";
            if($blog['listing_image']){
                echo "<a href='".IMG_PATH.'listingimage/'.$blog['listing_image']."' target='_blank'>".$this->Html->image(IMG_PATH.'listingimage/'.$blog['listing_image'], ['width' =>'250','height' =>'100'])."&nbsp;&nbsp;&nbsp;View</a></td></tr>";
            }
            ...
            if($blog['banner_image']){
                echo "<a href='".IMG_PATH.'bannerimage/'.$blog['banner_image']."' target='_blank'>".$this->Html->image(IMG_PATH.'bannerimage/'.$blog['banner_image'], ['width' =>'250','height' =>'100'])."&nbsp;&nbsp;&nbsp;View</a></td></tr>";
            }
...
<script>
 CKEDITOR.replace( 'post', {
  height: 300,
  filebrowserUploadUrl: "http://server:81/current/layeredvastu.com/upload.php"
 });
</script>